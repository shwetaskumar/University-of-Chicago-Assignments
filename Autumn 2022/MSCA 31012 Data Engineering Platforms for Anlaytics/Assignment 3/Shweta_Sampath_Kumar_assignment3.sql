...
SELECT 
	DISTINCT c.city
	FROM 
    sakila.address a LEFT JOIN sakila.city c ON a.city_id = c.city_id;
...
FROM 
  sakila.rental 
WHERE 
  DATE(rental_date) BETWEEN '2005-02-01' 
  AND '2005-05-31';
...
WHERE 
  (replacement_cost > 15) 
  OR (
    rental_duration BETWEEN 6 
    AND 10
  );
# 3d) What is the total amount spent by customers to rent movies in the year 2005? Alias the result column as total_spent.
SELECT 
    SUM((sakila.film.rental_rate / sakila.film.rental_duration)* DATEDIFF(return_date, rental_date)) AS total_spent 
FROM 
  sakila.rental 
...
WHERE 
  YEAR(rental.rental_date) = 2005 AND YEAR(rental.rental_date) IS NOT NULL;
